...
TAG:= "astronomer-starship-v$(VERSION)"
...
test-ship: ## ship to test pypi dist
	$(TWINE) upload --repository testpypi dist/astronomer-starship-${VERSION}.tar.gz -u ${TWINE_USER} -p ${TWINE_TEST_PASS}

ship: ## ship pypi dist
ship: package
	$(TWINE) upload dist/astronomer-starship-${VERSION}.tar.gz -u ${TWINE_USER} -p ${TWINE_PASS}
